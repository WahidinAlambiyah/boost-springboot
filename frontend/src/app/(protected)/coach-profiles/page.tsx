...
export default function CoachProfilesPage() {
  ...
  const createMutation = useMutation({ mutationFn: (values: CoachProfileFormSubmitValues) => coachProfileService.create(values), onSuccess: invalidateProfiles });
  const updateMutation = useMutation({ mutationFn: ({ id, values }: { id: string; values: CoachProfileFormSubmitValues }) => coachProfileService.update(id, values), onSuccess: async () => { setSelectedProfile(null); await invalidateProfiles(); } });
  const deleteMutation = useMutation({ mutationFn: (id: string) => coachProfileService.remove(id), onSuccess: invalidateProfiles });

  return <RequirePermission permissions="COACH_READ"><AppShell><h1 className="text-2xl font-semibold text-zinc-900">Coach Profiles</h1><div className="mt-4 rounded-lg border border-zinc-200 bg-white p-4"><label className="block text-sm"><span className="mb-1 block font-medium text-zinc-700">Filter Academy</span><select value={academyId} onChange={(event) => setAcademyId(event.target.value)} className="w-full rounded-md border border-zinc-300 px-3 py-2"><option value="">Semua academy</option>{academiesQuery.data?.map((academy) => <option key={academy.id} value={academy.id}>{academy.name}</option>)}</select></label></div><div className="mt-6">{profilesQuery.data ? <CoachProfileTable profiles={profilesQuery.data} academies={academiesQuery.data ?? []} canWrite={canWrite} onEdit={setSelectedProfile} onDelete={(profile) => deleteMutation.mutate(profile.id)} /> : null}</div><div className="mt-6 rounded-lg border border-zinc-200 bg-white p-4"><h2 className="text-lg font-semibold text-zinc-900">{selectedProfile ? "Edit Coach" : "Tambah Coach"}</h2><div className="mt-3"><CoachProfileForm academies={academiesQuery.data ?? []} users={usersQuery.data ?? []} initialData={selectedProfile} canWrite={canWrite} isSubmitting={createMutation.isPending || updateMutation.isPending} onCancelEdit={() => setSelectedProfile(null)} onSubmit={(values) => selectedProfile ? updateMutation.mutate({ id: selectedProfile.id, values }) : createMutation.mutate(values)} /></div></div></AppShell></RequirePermission>;
}
