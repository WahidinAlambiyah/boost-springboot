...
export default function NotificationPage() {
  ...
  const notificationQuery = useQuery({
    ...
    queryFn: notificationService.getSummaries,
  });
  ...
  return (
    <RequirePermission permissions={["NOTIFICATION_READ", "NOTIFICATION_WRITE"]} mode="any">
      <AppShell>
        ...
        {notificationQuery.isLoading ? <LoadingSkeleton rows={4} /> : null}
        ...
        {notificationQuery.data?.length === 0 ? (
          <EmptyState
            title="Data kosong"
            description="Belum ada data notifikasi yang dapat ditampilkan."
          />
        ) : null}
