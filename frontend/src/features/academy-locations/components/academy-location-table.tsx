  ...
  return (
    <div className="overflow-hidden rounded-lg border border-zinc-200 bg-white">
      <table className="w-full text-left text-sm">
        ...
        <tbody>
          {locations.map((location) => <tr key={location.id} className="border-t border-zinc-200 text-zinc-800"><td className="px-4 py-2">{academyMap.get(location.academyId) ?? "-"}</td><td className="px-4 py-2">{location.code}</td><td className="px-4 py-2">{location.name}</td><td className="px-4 py-2">{location.address || "-"}</td><td className="px-4 py-2">{location.isActive ? "Yes" : "No"}</td>{canWrite ? <td className="px-4 py-2"><div className="flex gap-2"><button type="button" className="rounded border border-zinc-300 px-2 py-1" onClick={() => onEdit(location)}>Edit</button><button type="button" className="rounded border border-red-300 px-2 py-1 text-red-700" onClick={() => onDelete(location)}>Delete</button></div></td> : null}</tr>)}
        </tbody>
      </table>
    </div>
  );
